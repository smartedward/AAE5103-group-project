...
class mtcnn():
    def __init__(self):
        self.Pnet = create_Pnet('D:/pycharm/opencv/keras-face-recognition-master/model_data/pnet.h5')
        self.Rnet = create_Rnet('D:/pycharm/opencv/keras-face-recognition-master/model_data/rnet.h5')
        self.Onet = create_Onet('D:/pycharm/opencv/keras-face-recognition-master/model_data/onet.h5')
    ...
    def detectFace(self, img, threshold):
        ...
        #   计算原始输入图像
        #   每一次缩放的比例
        #   scales是一个列表，里面存放的是制作图片金字塔每一次的缩放比例
        #-----------------------------#
        ...
        for scale in scales:
            ...
            # 这是在np数组中在第一个位置添加一个维度，用于表示批量的大小，添加的批量（batch）大小默认为1，
            # 批量大小表示每次传入输入数据的个数，在这里就是图片的个数
            inputs = np.expand_dims(scale_img, 0)
            # self.Pnet 返回的是一个模型，而这个模型的输入，和输出已经在self.Pnet中定义了
        ...
        for i in range(len(scales)):
            ...
            #   cls_prob = out[i][0][:, :, 1]代表这每一个尺度每个网格的置信度，roi = out[i][1]代表每个尺度每个网格的候选框位置
            #------------------------------------------------------------------#
            cls_prob = out[i][0][:, :, 1]
            roi = out[i][1]
            #-------------------------------------#
